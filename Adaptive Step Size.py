...
def orbit():
    ...
    while current_time < total_time:
        ...
        ###Your code here.
        #Local Truncation 
        lteE= numpy.linalg.norm(xE-x) + (total_time* numpy.linalg.norm(vE-v))
        h_new = h_new*(tolerance/(lteE + 1**(-50)))**(.5) # safe variant: / (error + e-50)
            # for safety add: h_new = min(1800., max(0.1, hnew))
